        ...
        <section class="card">
            ...
            <div class="card-body">
                <table class="table table-bordered table-striped mb-0" id="datatable-editable">
                    ...
                    <tbody>
                        @php
                            \Carbon\Carbon::setlocale('sq');
                        @endphp
                    @forelse($registers as $register)
                        <td>{{$register->client->full_name}}</td>
                        <td>{{$register->car->model.' '.$register->car->marsh.' - '.$register->car->color}}</td>
                        <td>Nisja: {{\Carbon\Carbon::parse($register->start_date)->isoFormat('D MMMM YYYY, hh:mm')}} <br> Kthimi: {{\Carbon\Carbon::parse($register->end_date)->isoFormat('D MMMM YYYY, hh:mm')}}</td>
                        <td>
                        @if(\Carbon\Carbon::parse($register->end_date) <= \Carbon\Carbon::now())
